...
void main() {
  group('Fetch from Wikipedia API', () {
    test('Fetches cat page from wikipedia', () async {
      final Summary summary =
          await WikipediaApiClient.getArticleSummary('cat');
      expect(summary, isNotNull);
      expect(
        summary.titles.normalized.toLowerCase(),
        'cat',
      );
    });

    test(
      'Fetches random article summary from wikipedia',
      () async {
        final Summary summary =
    ...
    test(
      "Fetches 'on this day' timeline from Wikipedia",
      () async {
    ...
    test('Fetches the feed', () async {
      final WikipediaFeed feed =
          await WikipediaApiClient.getWikipediaFeed();
  ...
  group('Fetch from wikimedia api', () {
    test(
      'Fetches cat article extract from wikipedia',
      () async {
        final List<Article> articles =
            await WikimediaApiClient.getArticleByTitle(
              'cat',
            );
    ...
    test(
      "Handles disambiguation page when article title doesn't match a "
    ...
    test('Handles searching', () async {
      final SearchResults searchResults =
          await WikimediaApiClient.search('dart');
